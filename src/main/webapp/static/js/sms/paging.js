...
(function(root, factory) {
	...
	Paging.prototype = {
		...
		bindToolbar: function() {
			var _this = this;
			var html = $('<li class="ui-paging-toolbar"><select class="ui-select-pagesize"></select><input type="hidden" class="ui-paging-count"/><a href="javascript:void(0)">跳转</a></li>');
			...
			$('a', html).click(function() {
				...
				_this.go(current);
				//$('.ui-select-pagesize option').each(function(i,val){
				//	val.attr("selected",false);
				//	if(val.text()==current){
				//		val.attr("selected",true);
				//	}
				//})
				//sel.find("option[value='"+current+"']").attr('selected',true);
			});
			sel.change(function() {
				//_this.changePagesize($(this).val());
				$('input',html).val($(this).val())
			});
