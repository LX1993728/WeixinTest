...
import com.example.core.utils.FileUploadUtils;
import java.io.*;
...
public class FileUploadController {
	...
	private String uploadAndGetPath(MultipartFile file) {
		...
		// 注意：随机目录可能不存在,需要创建
		File rd = new File(Global.imgDirectory, randomDirectory);
		if (!rd.exists()) {
			rd.mkdirs();
		}
		...
		return new File(rd, uuidname).getPath().substring(Global.imgDirectory.length() - 1);
	}
